...
import { moderateProduct } from '@/lib/product-store';
...
export async function POST(
  ...
  try {
    ...
    await moderateProduct(viewer.userId, id, action, reason);
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to moderate product' },
      { status: 500 },
    );
  }
}
